...
```
Либо использовать Python 3.13 интерпретатор в PyCharm. 

3. Запустить сервер через терминал:
```bash
uvicorn main:app --reload --host 0.0.0.0 --port 8000
```
Если всё успешно увидите строчку `Uvicorn running on http://0.0.0.0:8000 (Press CTRL+C to quit)`
4. При переходе по этой ссылке в браузере вы должны увидеть сообщение от FastAPI: `{"db" : "ok"}` - значит соединение с БД установлено успешно.
5. Для перехода в Swagger UI добавьте в адресную строку: `http://<ваш_ip>:8000/docs`
...
После успешного запроса должна создаться папка `logs/` с файлом лога лучшего порядка, в нем содержится подробная информация о стартах задач и их последовательности.
...
## Полезные файлы
- `main.py` — точка входа FastAPI и все ручки  
